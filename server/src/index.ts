...
app.use(
  cors({
    origin: (origin, callback) => {
      // Requests without an Origin header, such as direct API calls,
      // Postman, curl, or Render health checks, are allowed.
      if (!origin) {
...
app.get("/api/planner", async (req, res) => {
  try {
    ...
    const [cutsResult, jobsResult, settingsResult] =
      await Promise.all([
        pool.query(
          `
          SELECT *
    ...
    if (!settings) {
      return res.status(500).json({
        error: "Planner settings were not found.",
      });
    }
    ...
    const generatorMinutes = scheduled.reduce(
      (sum, job) => sum + job.generatorMinutes,
      0
    );

    const generatorCostPerHour = Number(
      settings.generator_cost_per_hour
    );
    ...
    res.json({
      ...
      summary: {
        ...
        estimatedGeneratorCost:
          (generatorMinutes / 60) *
          generatorCostPerHour,

        totalScheduledMinutes: scheduled.reduce(
          (sum, job) =>
      ...
      settings: {
        shopStartMinute: Number(
          settings.shop_start_minute
        ),

        shopEndMinute: Number(
          settings.shop_end_minute
        ),
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      const start = timeToMinute(
        body.shopStartTime
      );

      const end = timeToMinute(
        body.shopEndTime
      );
...
app.listen(
  ...
  () => {
    console.log(
      `PowerPlan API running on port ${port}`
    );
